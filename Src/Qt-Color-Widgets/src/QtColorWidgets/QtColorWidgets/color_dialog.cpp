    ...
    : QDialog(parent, f), p(new Private)
{
    ...
    connect(QGuiApplication::primaryScreen(),
            &QScreen::primaryOrientationChanged, this,
    ...
    connect(p->ui.wheel, &ColorWheel::colorSpaceChanged, this,
            &ColorDialog::colorSpaceChanged);
    connect(p->ui.wheel, &ColorWheel::selectorShapeChanged, this,
            &ColorDialog::wheelShapeChanged);
    ...
    connect(p->ui.spin_cyan,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            &ColorDialog::set_cmyk);
    connect(p->ui.spin_magenta,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            &ColorDialog::set_cmyk);
    connect(p->ui.spin_yellow,
            QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
            &ColorDialog::set_cmyk);
...
void ColorDialog::setCmykColor(const QColor &color, double c, double m,
                               double y, double k)
{
...
void ColorDialog::set_cmyk()
{
    if (!signalsBlocked()) {
        ...
        p->ui.spin_red->setValue(p->ui.slide_red->value());
        p->ui.slide_red->setFirstColor(
            QColor(0, newColor.green(), newColor.blue()));
        p->ui.slide_red->setLastColor(
            QColor(255, newColor.green(), newColor.blue()));

        p->ui.slide_green->setValue(newColor.green());
        p->ui.spin_green->setValue(p->ui.slide_green->value());
        p->ui.slide_green->setFirstColor(
            QColor(newColor.red(), 0, newColor.blue()));
        p->ui.slide_green->setLastColor(
            QColor(newColor.red(), 255, newColor.blue()));

        p->ui.slide_blue->setValue(newColor.blue());
        p->ui.spin_blue->setValue(p->ui.slide_blue->value());
        p->ui.slide_blue->setFirstColor(
            QColor(newColor.red(), newColor.green(), 0));
        p->ui.slide_blue->setLastColor(
            QColor(newColor.red(), newColor.green(), 255));
        ...
        p->ui.slide_saturation->setValue(
            qRound(p->ui.wheel->saturation() * 100.0));
        p->ui.spin_saturation->setValue(p->ui.slide_saturation->value());
        p->ui.slide_saturation->setFirstColor(
            QColor::fromHsvF(p->ui.wheel->hue(), 0, p->ui.wheel->value()));
        p->ui.slide_saturation->setLastColor(
        ...
        p->ui.slide_alpha->setFirstColor(
            QColor(newColor.red(), newColor.green(), newColor.blue(), 0));
        p->ui.slide_alpha->setLastColor(
            QColor(newColor.red(), newColor.green(), newColor.blue(), 255));
...
void ColorDialog::set_rgb()
{
    if (!signalsBlocked()) {
        QColor col(p->ui.slide_red->value(), p->ui.slide_green->value(),
                   p->ui.slide_blue->value(), p->ui.slide_alpha->value());
        if (col.saturation() == 0)
...
void ColorDialog::on_buttonBox_clicked(QAbstractButton *btn)
{
    ...
    case QDialogButtonBox::ApplyRole:
        ...
        if (p->pendingCmyk.valid)
            Q_EMIT colorSelectedCmyk(color(), p->pendingCmyk.c,
                                     p->pendingCmyk.m, p->pendingCmyk.y,
                                     p->pendingCmyk.k);
...
void ColorDialog::mouseReleaseEvent(QMouseEvent *event)
{
    if (p->pick_from_screen) {
        ...
        setColorInternal(
            utils::get_screen_color(event->globalPosition().toPoint()));
#else
