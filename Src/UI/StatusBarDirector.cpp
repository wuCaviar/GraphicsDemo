...
void StatusBarDirector::applyZoomFromEdit()
{
    ...
    qreal level = AtMath::clamp(pct / 100.0, 0.01, 32.0);
...
void StatusBarDirector::onPageSwitched(const QString &, const QString &pageType)
{
    ...
    if (pageType == QStringLiteral("canvas")) {
        ...
        if (p) {
            ...
            connect(p, &QAtCanvasPage::zoomChanged, this, &StatusBarDirector::onZoomChanged);
            connect(p, &QAtCanvasPage::toolChanged, this, &StatusBarDirector::onToolChanged);
        }
    }
}
