...
| 维度 | 发现问题数 | 严重/高/中/低 | 已修复 |
|------|-----------|--------------|--------|
| 核心架构 (Items/Commands) | 16 | 6 / 4 / 4 / 2 | 12 |
| UI 层 | 18 | 5 / 3 / 6 / 4 | 8 |
| 构建系统/工具链/测试 | 20 | 0 / 6 / 8 / 6 | 10 |
| **合计** | **54** | **11 / 13 / 18 / 12** | **30** |
...
### 2. RotationChangeCommand 构造函数修改活跃对象状态 [已修复]

**文件：** `Src/Commands/Commands.cpp:358-383`

构造函数临时修改 item 的旋转角度来计算补偿位置，然后恢复。如果构造期间发生异常，item 会停留在错误状态。

**修复方案：** 使用 `QTransform` 纯数学计算旋转补偿，不修改 item 状态。通过 `translate(origin).rotate(delta).translate(-origin)` 计算旋转后中心位置。
...
**修复方案：** JPEG 压缩方案使用 `7` (COMPRESSION_JPEG)，JPEG 质量通过 `setQuality()` 设置。

### 10. applyDpiToImage 是空函数 [已修复]

**文件：** `Src/Utils/ImageUtils.cpp:331-338`

所有参数标记 `Q_UNUSED`，函数体为空，但 Header 中有声明和文档。这是未完成的功能。

**修复方案：** 实现 DPI 转换，将 `params.dpi` 转为 dots-per-meter 设置到 QImage。

### 11. cloneItem() 缺少 transformOriginPoint 复制 [已修复]

**文件：** 所有 7 个 Item 的 `cloneItem()` 方法

复制了 `pos()`、`rotation()`、`transform()`，但遗漏了 `transformOriginPoint()`。用户设置自定义旋转中心后，克隆体旋转行为会与原件不同。

**修复方案：** 在每个 `cloneItem()` 中添加 `item->setTransformOriginPoint(transformOriginPoint())`。

### 12. 构建系统缺少编译器警告标志 [已修复]

**文件：** `Src/Src.pro`

没有任何 `-Wall`、`-Wextra`、`-Wpedantic` 配置。项目使用 `reinterpret_cast`、`dynamic_cast` 和底层 TIFF I/O，缺少警告等于盲飞。

**修复方案：** 添加 `QMAKE_CXXFLAGS += -Wall -Wextra -Wpedantic`，同时为测试项目添加。

### 13. 主构建未显式指定 C++ 标准 [已修复]
...
### 14. "ZIP Horizontal Differencing" 复选框是死 UI [已修复]

**文件：** `Src/UI/ExportImageDialog.cpp:90-91`

复选框创建为局部变量，值从未被读取。用户交互完全无效。

**修复方案：** 将复选框存储为成员变量 `m_zipDiffCheck`，在 `getParameters()` 和 `setParameters()` 中读写其值。
...
| # | 问题 | 位置 | 状态 |
|---|------|------|------|
| 15 | Command 中并行列表访问无范围检查，尺寸不匹配时越界 | Commands.cpp:110,276,314 | 待修复 |
| 16 | PropertyChangeCommand Geometry 分支用 qgraphicsitem_cast 类型分发，违反开闭原则 | Commands.cpp:166-175 | ✅ 已修复 |
| 17 | 每次 undo/redo 都 dynamic_cast，应缓存结果 | Commands.cpp:157,190 | 待修复 |
| 18 | cloneItem() 返回裸指针，违反 RAII | 所有 Item.cpp | 待修复 |
| 19 | filterSelectableItems() 逻辑在 6 个位置重复 | mainwindow/qatgraphicsview/AlignLayoutDialog | ✅ 已修复 |
| 20 | QDataStream 未设置显式版本号 | mainwindow.cpp:1309,1349 | ✅ 已修复 |
| 21 | 分布算法 gap 为负数时未处理 | AlignmentUtils.cpp:191,235 | 待修复 |
| 22 | TIFF 尺寸上限 50000 允许 10GB 分配 | ImageUtils.cpp:69 | 待修复 |
| 23 | PropertyPanel 同一操作发出两个 undo 命令 | PropertyPanel.cpp:720-752 | 待修复 |
| 24 | RulerBar 硬编码 macOS 字体 "SF Pro Display" | RulerBar.cpp:213,272,349 | 待修复 |
| 25 | closeEvent 和析构函数重复清理逻辑 | mainwindow.cpp:1490-1508, 100-116 | 待修复 |
| 26 | ExportImageDialog::getSelectedFormat() 始终返回空字符串 | ExportImageDialog.cpp:144-148 | 待修复 |
| 27 | preserveICCProfile 收集但从未使用 | ExportImageDialog.cpp:118 | 待修复 |
| 28 | 硬编码 /opt/homebrew 路径（仅 ARM Mac） | Common.pri:5-6 | 待修复 |
| 29 | .gitignore 中 Build* 不匹配小写 build/ | .gitignore:93 | ✅ 已修复 |
| 30 | .DS_Store 和构建产物被 git 跟踪 | Tests/ | ✅ 已修复 |
...
| # | 问题 | 位置 | 状态 |
|---|------|------|------|
| 31 | ResizeHandleItem 析构函数可改为 = default | ResizeHandleItem.cpp:31 | 待修复 |
| 32 | CanvasItem 硬编码 Z 值 -9999 | CanvasItem.cpp:12 | 待修复 |
| 33 | CanvasItem::setPpi() 不通知依赖方 | CanvasItem.cpp:31 | 待修复 |
| 34 | .clang-tidy 禁用了过多安全检查 | .clang-tidy | ✅ 已修复 |
| 35 | ExportImageDialog::onFormatChanged 与 onCompressionChanged 冲突 | ExportImageDialog.cpp | 待修复 |
| 36 | 菜单栏和工具栏重复注册 Cut/Copy/Paste 快捷键 | mainwindow.cpp:195-211, 320-335 | 待修复 |
| 37 | GradientPreview 使用裸 new/delete 而非 unique_ptr | GradientDialog.cpp:28,36 | 待修复 |
| 38 | AlignLayoutDialog 使用 sender() 模式，不如 lambda 安全 | AlignLayoutDialog.cpp:255-269 | 待修复 |
| 39 | TIFF 元数据写入耦合在 MainWindow 而非 ImageUtils | mainwindow.cpp:734,816 | ✅ 已修复 |
| 40 | .qm 文件在 .gitignore 但运行时需要 | translations.qrc | 待修复 |
| 41 | ExportImageDialog 成员指针缺少默认初始化 | ExportImageDialog.h:35-44 | 待修复 |
| 42 | AlignLayoutDialog 静态成员持久化状态 | AlignLayoutDialog.h:75-78 | 待修复 |
...
`filterSelectableItems()` 逻辑曾在 6 个位置重复实现。已提取为 `IGraphicsItem.h` 中的 `filterSelectableItems()` 自由函数，所有调用方已更新。
...
### 6.1 .gitignore 修复建议 [已修复]

已添加 `build/`、`.DS_Store`、测试构建产物等规则。
...
已在 `Src/Src.pro` 和 `Tests/Tests.pro` 中添加：
```qmake
CONFIG += c++17
QMAKE_CXXFLAGS += -Wall -Wextra -Wpedantic
```

### 6.4 .clang-tidy 收紧 [已修复]
...
剪贴板序列化已有统一的版本/类型标记：
```
[版本号 (int)] [ItemCount (int)] [ItemType (int)] [Item数据...] ...
```
`copyItemsToClipboard()` 写入版本头，`pasteItemsFromClipboard()` 校验版本范围并逐项反序列化。

### 7.3 职责分离 [已修复]

`exportTiffLossless()`、`writeTiffMetadata()`、`exportImageWithParams()`、`applyColorSpaceForExport()` 已从 `mainwindow.cpp` 移入 `ImageUtils` 命名空间。`mainwindow.cpp` 不再包含 `#include <tiff.h>`。

### 7.4 PropertyChangeCommand 类型分发 [已修复]

Geometry 分支用 `qgraphicsitem_cast` 链判断具体类型，违反开闭原则。

**修复方案：** 在 `IGraphicsItem` 接口中添加 `virtual void setGeometryRect(const QRectF &rect)` 方法，在 RectItem、EllipseItem、TextItem、ImageItem 中实现。PropertyChangeCommand 的 Geometry 分支改为 `gi->setGeometryRect()`。
...
第二周（高质量）：
  ✦ 修复 cloneItem transformOriginPoint              ✅ 已完成
  ✦ 修复 ImageUtils 双重读取                          ✅ 已完成
  ✦ 修复 TIFF 压缩参数                                ✅ 已完成
  ✦ 添加编译器警告标志                                 ✅ 已完成
  ✦ 修复 .gitignore                                   ✅ 已完成
  ✦ 修复 applyDpiToImage 空函数                        ✅ 已完成
  ✦ 修复 ZIP Horizontal Differencing 死 UI            ✅ 已完成

第三周（架构改进）：
  ✦ 序列化版本号机制                              ✅ 已完成
  ✦ 提取 filterSelectableItems 为共享函数         ✅ 已完成
  ✦ TIFF I/O 从 MainWindow 移到 ImageUtils       ✅ 已完成
  ✦ 启用 clang-tidy 安全检查                      ✅ 已完成
  ✦ PropertyChangeCommand setGeometryRect 接口    ✅ 已完成
  ✦ RotationChangeCommand 纯数学旋转补偿         ✅ 已完成
